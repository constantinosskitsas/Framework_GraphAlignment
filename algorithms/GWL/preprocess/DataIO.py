...
def build_dict_mimic3(diagnose_dict_path: str,
    ...
    df_procedure = pd.read_csv(procedure_adm_path)
    ...
    diag2title = {}
    # , encoding="ISO-8859-1")#"utf8")
    df_diagnose = pd.read_csv(diagnose_dict_path)
    idx = 0
    ...
    proc2title = {}
    # , encoding="ISO-8859-1")#"utf8")
    df_procedure = pd.read_csv(procedure_dict_path)
    idx = 0
    ...
    diag_adm = {}
    # , encoding="ISO-8859-1")#"utf8")
    df_diagnose = pd.read_csv(diagnose_adm_path)
    for i, row in df_diagnose.iterrows():
        adm = str(row['HADM_ID'])
        icd = str(row['ICD9_CODE'])
        if icd in diag2idx.keys():
        ...
        if i % 10000 == 0:
            logger.info(
                '{}/{} rows are processed.'.format(i, len(df_diagnose)))
    ...
    for i, row in df_procedure.iterrows():
        ...
        if i % 10000 == 0:
            logger.info(
                '{}/{} rows are processed.'.format(i, len(df_procedure)))
